...
public class Configurator {
	...
	public static final String kClassName = "Configurator";
	...
	public static void main(String[] args) {
		...
		try {
			new Configurator( fileName, createOK );
		}
	...
	public Configurator( String inConfigFileName, boolean inCreateOK )
	...
	private static void __sep__High_Level_Operations__() {}
	//////////////////////////////////////////////////////////
	...
	public void renderForm( boolean inIsNew ) {
		...
		/*JFrame*/ mainframe = new JFrame();	// Title set below
		...
		mainTitle += ": " + (inIsNew ? "creating " : "editing ") + mSaveFileName;
		...
		for( Iterator sit = sections.iterator() ; sit.hasNext() ; ) {
			...
			for( Iterator fit = fields.iterator() ; fit.hasNext() ; ) {
				...
				// ==============================================
				String displayName = renderFieldLabelIfAny( p, field, type, rowCounter, gbc );
	...
	boolean verifyAll( boolean hasAlreadyCopiedSwing ) {
...
// JDOMHelper.writeToFile( pageElem, "tmp.xhtml", false );
	...
	DBConfig checkDB( boolean inDoSuccess ) {
		DBConfig outConfig = Configurator.this.formDbConfigFromSwingDataOrNull();
		//	System.err.println(
...
statusMsg( "input box action:", "Event: " + e );
	...
	void renderInlineButton(
		...
		String clickClassName = JDOMHelper.getStringFromAttributeTrimOrNull( inFieldElem, "click_event_class" );
		if( null!=clickClassName ) {

			try {

				// Class clickClass = Class.forName( clickClassName );
				final String kPrefix = "nie.config_ui.Configurator$Event_";
				...
				Class [] constrSig = new Class [] {
					nie.config_ui.Configurator.class
					// Object.class
				};
				...
				// 	);
				// Gives
				// "nie.config_ui.Configurator$Event_VerifyDBConn"
	...
	boolean clearSwingFormValue( String inFieldName ) {
		...
		if( widget instanceof JComboBox ) {
			...
			// comboWidget.setSelectedIndex(anIndex)
			// comboWidget.
			traceMsg( kFName, "Combo value = \"" + newValue + "\"" );
		}
		else if( widget instanceof JLabel ) {
			JLabel myLabel = (JLabel) widget;
			myLabel.setText( newValue );
			traceMsg( kFName, "Label value = \"" + newValue + "\"" );
		}
		else {
			JTextComponent textWidget = (JTextComponent) widget;
			textWidget.setText( newValue );
			traceMsg( kFName, "Text value = \"" + newValue + "\"" );
		}
		widget.invalidate();
		return true;
	}
	...
	boolean showYesNoDialog( String inTitle, String inMessage ) {
		int n = JOptionPane.showOptionDialog(
			mainframe,	// It's OK if mainframe is null
			inMessage + '\n' + '\n',
			inTitle,
