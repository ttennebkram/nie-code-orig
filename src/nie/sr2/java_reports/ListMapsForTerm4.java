 ...
 * Window>Preferences>Java>Code Generation>Code and Comments
 */
public class ListMapsForTerm4 extends BaseReport
{
			...
			throws ReportException
	{
		...
		// Subtitles are for the report sub title heading
		// To give more details about what the report is
		// subtitle is the local variable, and then cSubtitleB
		// is the object's cached final version
		...
		// If wildcard markers with *, use them
		if( null!=term && term.indexOf('*') >= 0 ) {
			term = NIEUtil.replaceChars( term, '*', '%' );
			termHadWildcards = true;
		}
		// Or if NO markers, add prefix and suffix
		else if( null!=term && term.indexOf('%') < 0 ) {
			term = "%" + term + "%";
			termHadWildcards = true;
		}
		...
		// Or if NO markers, add prefix and suffix
		else if( null!=keyUrl && keyUrl.indexOf('%') < 0 ) {
			keyUrl = "%" + keyUrl + "%";
			keyUrlHadWildcards = true;
		}

		// done with worrying about subtitle, store whatever we have
		cSubtitleB = subtitle;
		...
		// Form the query
		// PostgreSQL example:
		// SELECT DISTINCT m.id AS map_id
		// FROM nie_map m, nie_term t, nie_map_term_assoc mta
		// WHERE mta.term_id = t.id AND mta.map_id = m.id
		// AND t.text_normalized LIKE '%asp%'
		// ORDER BY m.id DESC

		// TODO: when they look for term, we should also search alt_terms
		// BUT this would require outer joins which are DB specific
		// AND if we just add another field on the form as a workaround
		// then there's a bunch of logic that allows for 2 states (term XOR URL) that
		// would need to be upgraded to handle 3 states
	...
	// Subtitle is the sub title for the report heading display
	String cSubtitleB;
