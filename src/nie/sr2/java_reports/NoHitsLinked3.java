 ...
 * Window>Preferences>Java>Code Generation>Code and Comments
 */
public class NoHitsLinked3 extends BaseReport
{
	public String kClassName()
	{
		return "NoHitsLinked3";
	}
			...
			throws ReportException
	{
		...
		String qry =
			...
			// We want percentage of no hits, right?  per miles
			+ " AND (NUM_RESULTS = 0 OR NUM_RESULTS IS NULL)"
			;
		...
		try {
			...
			while( ! atEndOfRecords )
			{
				...
				***/
				...
				// Don't pester the database more than we need to
				...
				***/
				...
				// Well, in this report we know it's zero!
				// But we keep the int around for the logic below which
				// we borrowed from PopSearchesLinked
				...
				***/
				...
				/* Add the Hyperlink or Drop down actions */
				//////////////////////////////////////
				//////////////////////////////////////
				//////////////////////////////////////
				...
				if( ! isNullSearch /*&& false*/ ) {
					...
					// If no user classes, use CLASSIC action links
					if( ! hasUserClasses ) {
						...
						// The link text and mode (Create or Edit)
						String linkText2 = hasClassicSuggestions ?
							ReportConstants.EDIT_TEXT_FANCY : ReportConstants.CREATE_TEXT_FANCY
							;
						String mode = hasClassicSuggestions ? nie.webui.UILink.UI_OPERATION_EDIT : nie.webui.UILink.UI_OPERATION_ADD;
						// Create the link
						Element linkElem2 = uiLink.generateLinkElement(inRequestObject, linkText2, mode, lQueryTerm );
						tmpElem.addContent( linkElem2 );

					}
					// Else does have user classes, use drop downs
					else {
					...
					}	// End Else does have user classes, use drop downs
			...
			if( totalSearches > 0 ) {
				...
				String leftMsg = "No-Hits Coverage:";
	...
	{
		// return "Popular Searches with NO Matching Pages";
		return "Popular Searches with NO Results";
	}
	...
	static final int _DEFAULT_ICON_WIDTH = 16;
	static final int _DEFAULT_ICON_HEIGHT = 16;
