 ...
 */
public class LoggingLink
{

	private final static String kClassName = "LoggingLink";
		...
		throws SearchLoggerException
	{
		...
		***/
		...
		if( SearchLogger.TRANS_TYPE_ADVERTISEMENT_CLICK_THROUGH == fTransType )
		...
		throws SearchLoggerException
	{
		...
		if( SnRequestHandler.SN_CONTEXT_CLICKTHROUGH_PROXY==inContext )
			_setPassRawArgs();	
...
//		}
		...
		// Must encode any HTTP stuff
		// This method is pretty aggressive and replaces any
		// slash, colon, dot, etc into percent seq
		String newURL = NIEUtil.encodeUrlAllPunctOrNull( inURL );
	...
	// Usually we will encode the destination URL
	// Once in a while we pass the args through as is
	// like when going back through a search engine
	private void _setPassRawArgs()
	{
		fPassRawArgs = true;
	}
	private boolean _getPassRawArgs()
	{
		return fPassRawArgs;
	}

	public void setResultsOrdinalOnThisPage( int inNum )
	{
		fResultsOrdinalOnThisPage = inNum;
	}
	public int getResultsOrdinalOnThisPage()
	{
		return fResultsOrdinalOnThisPage;
	}

	public void setResultsPageNumber( int inNum )
	{
		fResultsPageNumber = inNum;
	}
	public int getResultsPageNumber()
	{
		return fResultsPageNumber;
	}

	public void setResultsOrdinalInAllResults( int inNum )
	{
		fResultsOrdinalInResults = inNum;
	}
	public int getResultsOrdinalInAllResults()
	{
		return fResultsOrdinalInResults;
	}
...
//	}
	...
	// What context (usually log a link)
	private String fLinkContext;
	...
	// Other data they might want us to store
	private int fResultsPageNumber = -1;
	// How far along on THIS PAGE did the link appear
	// reserved for results list links and set by caller
	private int fResultsOrdinalOnThisPage = -1;
	// How far in the complete results
	private int fResultsOrdinalInResults = -1;

	// have we set the sn_context to direct logging
	private boolean fHaveSetContext;
	// Variables indicating what we've seen to date
	private boolean fHaveSetTransType;
	...
	// Once in a while we pass the args through as is
	// like when going back through a search engine
	private boolean fPassRawArgs = false;
