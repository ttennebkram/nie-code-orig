...
public class UserDataConsolidator extends Processor
{
	public String kClassName() { return "UserDataConsolidator"; }
	...
	////////////////////////////////////////////////////////////
	public UserDataConsolidator( Application inApplication,
		Queue[] inReadQueue,
		Queue[] inWriteQueue,
		Queue[] inUsesQueue,
		Element inParameter,
		String inID )
	{
		...
		if( (inReadQueue == null) || (inReadQueue.length != 1) ||
			(inReadQueue[0] == null) )
		{	
			
			fatalErrorMsg( kFName, 
			    "requires one input queue: data to consolidate" );
			System.exit( -1 );
		}
		...
		if( (inWriteQueue == null) || inWriteQueue.length!=1 || (inWriteQueue[0] == null) )
		{
			
			// System.err.println( "No output queue specified." );
			fatalErrorMsg( kFName, "No/invalid output queue specified." );
			System.exit( -1 );
		}

		fWriteQueue = inWriteQueue[0];

		if( (inUsesQueue == null) || inUsesQueue.length!=1 || (inUsesQueue[0] == null) )
		{
	...
	*************************************************/
	...
	String _getCheckKeyFieldName()
	{
		
		final String kFName = "_getCheckKeyFieldName";
		if( fHaveDoneInit )
		...
		if( tmpString == null || tmpString.trim().equals("") )
		{
			tmpString = _getMasterKeyFieldName();
		}
...
//	}
	...
	boolean _getDoIgnoreEmptyStrings()
	{
		if( fHaveDoneInit )
...
statusMsg( kFName,
				"Phase 2: process regular work units"
				);

			try {
			    mNewWorkUnit = new WorkUnit();
			}
			catch( Exception e ) {
			    fatalErrorMsg( kFName, "error creating blank work unit" );
			    stackTrace( kFName, e, "work unit creation error" );
			    System.exit(1);
			}
			...
			// while( true )
			{
				...
				if(trace) traceMsg( kFName, "Phase 2: Top of loop" 	);
				...
				{
					/*mWorkUnit*/ mNewWorkUnit.stackTrace( this, kFName, e, "unable to process work unit"
							+ ", marking invalid"
							);
					//continue;
					/*mWorkUnit*/ mNewWorkUnit.setIsValidRecord( false );
				}
				...
				//	// + ", lWorkUnit=" + lWorkUnit
				//	);
				// enqueue( fWriteQueue, mNewWorkUnit );
				// debugMsg( kFName,
				//	"back from enqueue"
				//	);
...
statusMsg( kFName, "bottom of run, about to enqueue results" );
		enqueue( fWriteQueue, mNewWorkUnit );
	}

	private void _saveMasterUnit( WorkUnit inWU )
	{
		final String kFName = "saveMasterUnit";
		List keys = inWU.getUserFieldsText( _getMasterKeyFieldName() );
		...
		while( it.hasNext() )
		{
			...
			if( key == null )
			{
				...
				mWorkUnit.errorMsg( this, kFName, "found a truly null field"
						+ " for key " + _getMasterKeyFieldName()
						+ ", skipping this key."
						);
				continue;
			}
			...
			if( ! _getIsCaseSensitive() )
				key = key.toLowerCase();
			// Do trimming if requested
			if( _getDoTrimStrings() )
			{
				key = key.trim();
			}
			// Ignore empty strings if requested to do so
			if( _getDoIgnoreEmptyStrings() )
				if( key.equals("") )
					continue;
			...
			if( _getIsNormalHashLogic() )
			{
				...
				if( fMainHashTableWorkUnits.containsKey( key ) )
				{
					...
					mWorkUnit.errorMsg( this, kFName, "Have already seen this master key: \"" + key + "\""
							+ ", for key field " + _getMasterKeyFieldName()
							+ ", not including this later work unit in master list."
							);
					continue;
				}
	...
	void grabData( WorkUnit inWU )
	{
		final String kFName = "grabData";
		// Sanity check
		if( inWU == null )
		{
		...
		// List candidateFields = inWU.getUserFields( _getCheckKeyFieldName() );
		List candidateFields = inWU.getUserData().getChildren();
		if( candidateFields == null || candidateFields.size() < 1 )
		{
			mWorkUnit.debugMsg( this, kFName,
				"no matching fields to check"
				+ " for key " + _getCheckKeyFieldName()
				);
			return;
		}
		...
		for( Iterator it = candidateFields.iterator() ; it.hasNext() ; )
		{
			...
			// mNewWorkUnit.addElement( null, field );
			mNewWorkUnit.getUserData().addContent( field );
	...
	void _lookupUnit( WorkUnit inWU, boolean doDeletes, boolean invertLogic )
	{
		...
		for( Iterator it = candidateFields.iterator() ; it.hasNext() ; )
		{
			...
			if( _getIsNormalHashLogic() )
			{
				...
				mWorkUnit.debugMsg( this, kFName,
					" key: \"" + key + "\""
					+ " foundKey: " + foundKey
					+ ", for key field " + _getCheckKeyFieldName()
					);

				if( _getAuditFieldName() != null )
				{
			...
			if( _getAuditFieldName() != null )
			{
				...
				inWU.addNamedField( _getAuditFieldName(), auditMatchText	);
			}
