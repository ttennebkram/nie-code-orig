...
public class URLBuilder extends Processor {
	...
	public void run()
	{
		...
		try
		{
			while( true )
			{
				...
				Element lParent = mWorkUnit.findElement( WorkUnit.USER_DATA_PATH + "/" + fParentFieldName );
				Element lFragment = mWorkUnit.findElement( WorkUnit.USER_DATA_PATH + "/" + fFragmentFieldName );
				...
				else
				{
					...
					catch( MalformedURLException mfue )
					{
						...
						mWorkUnit.stackTrace( this, kFName, mfue, "Error parsing url" );
				...
				mWorkUnit.getUserData().addContent( lNewElement );
	...
	{
		final String kFName = "cleanDoubleSlash";
		...
		// Else yes there is something for us to do
			...
			//	inURL + "'"
			//	);
			mWorkUnit.infoMsg( this, kFName, "Start.  url='" +
				inURL + "'"
				);
		...
		//	outURL + "'"
		//	);
		mWorkUnit.infoMsg( this, kFName, "End.  url='" +
				outURL + "'"
				);
