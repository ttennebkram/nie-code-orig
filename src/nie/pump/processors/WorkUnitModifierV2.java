...
//						);
	...
	boolean processWorkUnit( WorkUnit inWU )
	{
		...
		for( Iterator it = fInstructions.iterator(); it.hasNext(); )
		{
			...
			// !!!! REMINDER !!!!
			// Add name below
			// Update dispatch logic here
			// write your code
			// AND DON'T FORGET to add an entry in
			// nie.pump.base.system.dpump_proc_def.xml

			if( lInstruction.equals( CLEAR_ERROR_INSTRUCTION ) )
				success = clearErrorStatus( lElement, inWU ) && success;
			else if( lInstruction.equals( CLEAR_ERROR_IF_SET_INSTRUCTION ) )
				success = clearErrorStatusIfNeeded( lElement, inWU ) && success;
			else if( lInstruction.equals( COPY_FIELD_INSTRUCTION ) )
			...
			else if( lInstruction.equals( DROP_URL_POUND_SUFFIX_IF_PRESENT ) )
				success = doDropURLSuffixIfPresent( lElement, inWU ) && success;
			...
			else if( lInstruction.equals( SPLIT_CSV_INSTRUCTION ) )
				success = doSplitCSV( lElement, inWU ) && success;	
	...
	private boolean clearErrorStatusIfNeeded( Element inElement, WorkUnit inWorkUnit )
	{
		return clearErrorStatus( inElement, inWorkUnit, false );
	}
	private boolean clearErrorStatus( Element inElement, WorkUnit inWorkUnit )
	{
		return clearErrorStatus( inElement, inWorkUnit, true );
	}
	private boolean clearErrorStatus( Element inElement, WorkUnit inWorkUnit, boolean inWarnIfClear )
	{
		final String kFName = "clearErrorStatus";
		if( inWarnIfClear && inWorkUnit.getIsValidRecord() )
			inWorkUnit.warningMsg( this, kFName, "No Error Status, or Error was already cleared" );
		inWorkUnit.setIsValidRecord( true );
		return true;
	}
	...
	{
		final String kFName = "moveUnderField";
		if( inWorkUnit.getIsUserDataFlat() ) {
			mWorkUnit.errorMsg( this, kFName,
	...
	private boolean doDropURLSuffixIfPresent( Element inCmdElement, WorkUnit inWU )
	{
		final String kFName = "doDropURLSuffixIfPresent";
		String lSourceFieldName = getSourceFieldName( inCmdElement );
		String lDestinationFieldName = getDestinationFieldName( inCmdElement );
		if( lSourceFieldName.equals( lDestinationFieldName ) )
		{
		...
		{
			String srcString = (String)it.next();
			String dstString = srcString;
			int poundAt = srcString.indexOf( '#' );
			if( poundAt > 0 )
			{
				dstString = srcString.substring( 0, poundAt );
				result = true;
			}
			else if( 0 == poundAt )
			{
...
//			);
	...
	private boolean doSplitCSV( Element inCmdElement, WorkUnit inWU )
	{
		final String kFName = "doSplitCSV";
		String lSourceFieldName = getSourceFieldName( inCmdElement );
		String lDestinationFieldName = getDestinationFieldName( inCmdElement );
		if( lSourceFieldName.equals( lDestinationFieldName ) )
		{
		...
		for( Iterator it = srcValues.iterator(); it.hasNext(); )
		{
			...
			List newVals = NIEUtil.parseCSVLine( srcString );
			for( Iterator it2 = newVals.iterator(); it2.hasNext(); )
			{
				String tmpVal = (String) it2.next();
				tmpVal = NIEUtil.trimmedStringOrNull( tmpVal );
				if( null!=tmpVal )
				{
					inWU.addNamedField( lDestinationFieldName, tmpVal );
					result = true;
				}
			}
		}
...
//	}
	...
	// !!!! REMINDER !!!!
	// Add names here
	// And add the dispatch logic in processWorkUnit
	// write your code
	// AND DON'T FORGET to add an entry in
	// nie.pump.base.system.dpump_proc_def.xml

	static final String CLEAR_ERROR_INSTRUCTION = "clear_error";
	static final String CLEAR_ERROR_IF_SET_INSTRUCTION = "clear_error_if_set";
	...
	static final String DROP_URL_POUND_SUFFIX_IF_PRESENT = "drop_url_pound_suffix_if_present";
	...
	static final String SPLIT_CSV_INSTRUCTION = "split_csv";
	...
	static final int DEFAULT_KEEP_WHICH = nie.pump.base.PumpConstants.DEFAULT_KEEP_WHICH;
